...
class Poller:
    ...
    def _reading_to_dict(
        ...
        return {
            ...
            "station_type": self.driver.station_model.name if self.driver.station_model else "unknown",
            "inside_temp": temp_f(reading.inside_temp),
            "outside_temp": temp_f(reading.outside_temp),
            "inside_humidity": reading.inside_humidity,
            "outside_humidity": reading.outside_humidity,
            "wind_speed": reading.wind_speed,
            "wind_direction": reading.wind_direction,
            "barometer": bar_inhg(reading.barometer),
            "rain_total": reading.rain_total,
            "rain_rate": reading.rain_rate,
            "solar_radiation": reading.solar_radiation,
            "uv_index": reading.uv_index / 10.0 if reading.uv_index is not None else None,
            "heat_index": temp_f(hi),
            "dew_point": temp_f(dp),
            "wind_chill": temp_f(wc),
            "feels_like": temp_f(fl),
            "theta_e": theta / 10.0 if theta is not None else None,
            "pressure_trend": trend,
        }
